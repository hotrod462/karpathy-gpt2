...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        #mask, but named in the OpenAI convention
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        for block in self.transformer.h:
            x = block(x)
        #forward the final laternorm and the classifier
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        #creat optim groups. Any parameters that is 2D will be weight decayed, otherwise no
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes):
        ...
        with open('pewdiepie_transcripts.txt', 'r') as f:
            text= f.read()
...
for step in range(max_steps):
    ...
    if master_process:
        print(f"step {step} | loss: {loss_accum.item()} | norm: {norm: .4f} | dt: {dt:.2f}ms, tok/sec: {tokens_per_sec}")
